...
const zseb_32_t zseb::crc32::table[ ZSEB_CRC32_TABLE ] = {
...
zseb_32_t zseb::crc32::entry( const zseb_16_t idx ){

   zseb_32_t val = idx;

   for ( zseb_16_t k = 0; k < ZSEB_CHARBIT; k++ ){
...
zseb_32_t zseb::crc32::update( const zseb_32_t crc, const char * data, const zseb_32_t length ){
   ...
   for ( zseb_32_t idx_data = 0; idx_data < length; idx_data++ ){
      ...
      work = table[ ( work ^ data[ idx_data ] ) & ZSEB_CRC32_MASK ] ^ ( work >> ZSEB_CHARBIT );

   }

   return ( work ^ ZSEB_CRC32_BITFLIP );
